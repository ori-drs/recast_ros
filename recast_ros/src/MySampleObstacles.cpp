...
struct MeshProcess : public dtTileCacheMeshProcess
{
	...
	inline MeshProcess() : m_geom(0) //, areaTypes(1, 0)
	{
	}
...
int MySampleObstacles::rasterizeTileLayers(
...
// 	{
// 		m_sample = (MySampleObstacles *)sample;
// 	}
...
MySampleObstacles::~MySampleObstacles()
{
...
void MySampleObstacles::handleSettings(const int &nodeSize)
{
	...
	m_navQuery->init(m_navMesh, nodeSize);
	/* 	}
...
void MySampleObstacles::handleTools()
{
...
void MySampleObstacles::handleDebugMode()
{
...
void MySampleObstacles::handleRender()
{
...
void MySampleObstacles::renderCachedTile(const int tx, const int ty, const int type)
{
	/* 	if (m_tileCache)
		drawDetail(&m_dd,m_tileCache,tx,ty,type); */
}

void MySampleObstacles::renderCachedTileOverlay(const int tx, const int ty, double *proj, double *model, int *view)
{
	/* if (m_tileCache)
		drawDetailOverlay(m_tileCache, tx, ty, proj, model, view); */
}

void MySampleObstacles::handleRenderOverlay(double *proj, double *model, int *view)
{
	/*	if (m_tool)
...
void MySampleObstacles::handleMeshChanged(class InputGeom *geom) //, const std::vector<char> &areaTypes)
{
...
dtStatus MySampleObstacles::addTempObstacle(const float *pos, const float &radi, const float &height)
{
	if (!m_tileCache)
		return DT_FAILURE;
	float p[3];
...
void MySampleObstacles::removeTempObstacle(const float *sp, const float *sq)
{
	if (!m_tileCache)
		return;
...
void MySampleObstacles::clearAllTempObstacles()
{
	if (!m_tileCache)
		return;
...
bool MySampleObstacles::handleBuild(const std::vector<char> &areaTypes, const int &nodeSize)
{
	...
	tcparams.maxObstacles = 1024; //128;
	...
	status = m_navQuery->init(m_navMesh, nodeSize);
	if (dtStatusFailed(status))
	{
...
void MySampleObstacles::saveAll(const char *path)
{
	if (!m_tileCache)
		return;
...
void MySampleObstacles::loadAll(const char *path)
{
