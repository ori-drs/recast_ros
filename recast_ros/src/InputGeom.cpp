...
// This program is updated version of InputGeom.cpp in original 'recastnavigation'


#define _USE_MATH_DEFINES
#include "recast_ros/InputGeom.h"
#include "recast_ros/ChunkyTriMesh.h"
...
void InputGeom::setBuildSettings(const BuildSettings& settings, bool replaceBounds)
{
  // update bounds on settings if they were never set
  if (!m_hasBuildSettings) {
    for (int i = 0; i < 3; i++) {
      m_buildSettings.navMeshBMin[i] = m_meshBMin[i];
      m_buildSettings.navMeshBMax[i] = m_meshBMax[i];
    }
  }
  // copy
  m_buildSettings.cellSize = settings.cellSize;
  m_buildSettings.cellHeight = settings.cellHeight;
  m_buildSettings.agentHeight = settings.agentHeight;
  m_buildSettings.agentRadius = settings.agentRadius;
  m_buildSettings.agentMaxClimb = settings.agentMaxClimb;
  m_buildSettings.agentMaxSlope = settings.agentMaxSlope;
  m_buildSettings.regionMinSize = settings.regionMinSize;
  m_buildSettings.regionMergeSize = settings.regionMergeSize;
  m_buildSettings.edgeMaxLen = settings.edgeMaxLen;
  m_buildSettings.edgeMaxError = settings.edgeMaxError;
  m_buildSettings.vertsPerPoly = settings.vertsPerPoly;
  m_buildSettings.detailSampleDist = settings.detailSampleDist;
  m_buildSettings.detailSampleMaxError = settings.detailSampleMaxError;
  m_buildSettings.partitionType = settings.partitionType;
  if (replaceBounds) {
    for (int i = 0; i < 3; i++) {
      m_buildSettings.navMeshBMin[i] = settings.navMeshBMin[i];
      m_buildSettings.navMeshBMax[i] = settings.navMeshBMax[i];
    }
  }
  m_buildSettings.tileSize = settings.tileSize;
  m_hasBuildSettings = true;
}

bool InputGeom::load(class rcContext* ctx, const pcl::PolygonMesh& pclMesh, bool needToRotateMesh)
{
	if (m_mesh)
	{
		delete m_chunkyMesh;
	...
	if (!m_mesh->load(pclMesh, needToRotateMesh))
	{
		ctx->log(RC_LOG_ERROR, "buildTiledNavigation: Could not load pcl::PolygonMesh");
		return false;
	}
